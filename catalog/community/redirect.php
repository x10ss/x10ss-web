...
<div class="row">
	...
	<div class='col-md-10'>
	<p>
	<form method='post'> acion='/catalog/community/?page=1&order=title&direction=desc&title="?>
	<script>
	doument.getElementById('title2').innerText
	</script>
	'"
	<?php 
	echo "
...
for($x = 0; $x < $arrlength; $x++) {
   ...
   $id= $rows[$x]["ID"];
	...
	<a style="font-size: 18px;" href="?page=1&order=hits&direction='.$newdirection5.'">'.$hits.'</a>
	<small>
	<img height="12px" src="/images//mouse.svg" /> '.
	$rows[$x]["Hits"]
	. ' | </small>
	<a style="font-size: 18px;"  href="?page=1&order=zipsize&direction='.$newdirection6.'">'.$zipsize.'</a>
	<img width="12px" src="/images/hard.svg" />
	<small>' . round(($rows[$x]["ZipSize"]) / 1024 , 1). ' megabytes</small>
	|
	<a style="font-size: 18px;" href="?page=1&order=date&direction='.$newdirection3.'">'.$date.'</a>
...
echo '<p style="text-align:center;" class="vimh"> | ';
	for( $i = 0; $i <= $result->num_rows/8; $i++ ) {
		$p=$i + 1;
		echo $_GET["page"]==$p? "<span style='color:black'>$p</span> | ":" <a href='?page=$p&order=$orderget&direction=$directionget'>$p</a> | ";
		}
echo '</p>';
} else {
  echo "0 results";
...
function getValue() {
    ...
    const inputElement = document.getElementById('searh');
    
    // Dohvati vrijednost koju korisnik trenutno upisuje
    const value = inputElement.value;

    // Prikaži vrijednost u <p>
    document.getElementById('title2').innerText = "Trenutna vrijednost: " + value;
}
</script>
	</body>
</html>
